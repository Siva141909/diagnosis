...
contact_info = st.text_input("Mobile Number")
...
if len(contact_info) != 10 or not contact_info.isdigit():
    valid_input = False
    validation_messages.append("Please enter a valid 10-digit mobile number.")

# Validate age
if age < 1:
    valid_input = False
    validation_messages.append("Age must be greater than or equal to 1.")

# Validate symptoms
selected_symptoms = st.multiselect("Select Symptoms (Minimum 3 symptoms)", all_symptoms)
if len(selected_symptoms) < 3:
    valid_input = False
    validation_messages.append("Please select at least 3 symptoms.")
...
# Show "Predict Disease" button only if inputs are valid
if valid_input:
    # Patient History
    ...
    if st.button("Predict Disease"):
        ...
        symptom_array = get_symptom_array(selected_symptoms, all_symptoms)
        
        # Predict disease
        prediction = rf_model.predict([symptom_array])
        predicted_disease = le.inverse_transform(prediction)[0]

        # Get diet and medication details for the predicted disease
        disease_info = get_diet_for_disease(predicted_disease)

        # Display the prediction and disease info
        ...
        patient_data = {
            ...
            'Age': age,
            'Gender': gender,
            'Mobile Number': contact_info,
            'Symptoms': ', '.join(selected_symptoms),
        }
        pdf_file_path = create_pdf(patient_data, disease_info)
        st.download_button(label="Download Patient Report", data=open(pdf_file_path, "rb").read(), file_name="patient_report.pdf")
else:
    st.write("Please fill in all required fields to enable disease prediction.")
