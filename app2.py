...
# Predict Disease (Mockup for demonstration)
if st.button("Predict Disease"):
    if name and age and gender and contact_info and selected_symptoms:
        ...
        # Get Disease Diet Information
        disease_info = get_diet_for_disease(predicted_disease)

        # Create PDF Report
        if st.button("Download Report"):
            patient_data = {"Name": name, "Age": age, "Gender": gender, "Contact Info": contact_info, "Symptoms": ", ".join(selected_symptoms)}
            pdf_file_path = create_pdf(patient_data, disease_info)
            st.success("Report created successfully. You can download it now.")
            st.download_button("Download PDF", pdf_file_path)
    else:
        st.error("Please fill in all the fields to predict the disease.")
